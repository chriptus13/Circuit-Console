...
public class Game {
    ...
    private static final String LEVELS_FILE = "Demo"; // Name of levels file
    ...
    private boolean loadLevel(int n) {
        Scanner in = null;
        try {
            in = new Scanner(new FileInputStream("Demo.txt")); // Scanner to read the file
            model = new Loader(in).load(n);                     // Load level from scanner
            // Adicionado para caso nao exista mais levels (ver class Loader, metodo load)
            // Dava exception no caso de nao encontrar mais levels, nao escrevendo na consola "No more levels".
            if (model == null) {
                return false;
            }
            return true;
    ...
    public void unHighLightFrom() {
        view.paint(from, false);
    }
